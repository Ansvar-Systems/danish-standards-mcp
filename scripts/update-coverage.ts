...
function parseSourcesYml(content: string): SourceMeta[] {
  ...
  for (const block of blocks) {
    ...
    const freqMatch = block.match(/(?:refresh_frequency|update_frequency):\s*"([^"]+)"/m);
...
async function main(): Promise<void> {
  console.log('Update Coverage — Danish Standards MCP');
  console.log('=======================================');
  ...
  if (existsSync(COVERAGE_MD)) {
    ...
    const newTotalLine = `**Total:** ${toolFiles.length} tools, ${totalControls} controls/requirements, database built from ${coverageSources.length} authoritative Danish sources.`;
