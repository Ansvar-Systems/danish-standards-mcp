...
export function handleGetIsoMapping(args: { iso_control?: string }) {
  ...
  if (rows.length === 0) {
    return errorResponse(
      `No Danish controls are mapped to ISO 27002 control '${isoControl}'.`,
      'NO_MATCH'
    );
  }
  ...
  lines.push(`## ISO 27002 Control ${isoControl} — Danish Framework Mapping`);
  lines.push('');
  ...
  return {
    ...
    _citation: buildCitation(
      `ISO 27002 ${isoControl}`,
      `ISO 27002 Control ${isoControl} — Danish Framework Mapping`,
      'get_iso_mapping',
