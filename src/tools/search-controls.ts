...
export function handleSearchControls(args: {
  ...
  for (const row of rows) {
    // Decision 1: EN preferred if available, fall back to DA (Danish-only controls never hidden)
  ...
  return {
    ...
    _citations: rows.map((row) =>
      buildCitation(
        `${row.control_number} (${row.framework_id})`,
        row.title_nl ?? row.title ?? row.control_number,
        'get_control',
        { control_id: row.id },
      ),
    ),
